...
## Key Files
- `backend/packages/server/api/src/app/ee/api-keys/api-key-module.ts` — module registration with `platformMustHaveFeatureEnabled` guard
- `backend/packages/server/api/src/app/ee/api-keys/api-key-entity.ts` — TypeORM entity
- `backend/packages/server/api/src/app/ee/api-keys/api-key-service.ts` — service (add, list, delete, lookup by value)
- `backend/packages/shared/src/lib/ee/api-key/index.ts` — `ApiKey`, `ApiKeyResponseWithValue`, `ApiKeyResponseWithoutValue`, `CreateApiKeyRequest` types
- `frontend/packages/web/src/features/platform-admin/api/api-key-api.ts` — frontend API client
- `frontend/packages/web/src/features/platform-admin/hooks/api-key-hooks.ts` — React query hooks
- `frontend/packages/web/src/app/routes/platform/security/api-keys/` — platform admin UI page
