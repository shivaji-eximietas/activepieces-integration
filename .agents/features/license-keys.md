...
## Key Files
- `backend/packages/server/api/src/app/ee/license-keys/license-keys-module.ts` — module registration, schedules daily `TRIAL_TRACKER` job
- `backend/packages/server/api/src/app/ee/license-keys/license-keys-controller.ts` — REST controller (get key info, verify and apply)
- `backend/packages/server/api/src/app/ee/license-keys/license-keys-service.ts` — service (validate, apply limits, downgrade, request trial)
- `backend/packages/shared/src/lib/core/license-keys/index.ts` — `LicenseKeyEntity`, `VerifyLicenseKeyRequestBody`, `CreateTrialLicenseKeyRequestBody` types
- `frontend/packages/web/src/features/billing/components/license-key.tsx` — frontend license key display component
- `frontend/packages/web/src/features/billing/components/activate-license-dialog.tsx` — activation dialog
- `frontend/packages/web/src/app/routes/platform/billing/index.tsx` — billing/license page
