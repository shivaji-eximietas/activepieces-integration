...
const main = async () => {
  ...
  const sharedDeps = ['backend/packages/pieces/framework', 'backend/packages/pieces/common']
  ...
  if (!sharedResults.every(p => p)) {
    validationResults.push(await packagePrePublishChecks('backend/packages/shared'))
  }
}
