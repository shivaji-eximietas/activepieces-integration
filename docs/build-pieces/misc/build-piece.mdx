...
The CLI scans the `backend/packages/pieces/` directory for the specified piece. It checks the **name** in the `package.json` file. If the piece is found, it builds and packages it into a `.tgz` archive.
...
```bash
Piece 'google-drive' built and packed successfully at backend/packages/pieces/community/google-drive/dist
```
