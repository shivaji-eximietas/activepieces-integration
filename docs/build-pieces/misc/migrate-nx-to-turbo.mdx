...
```bash
npx ts-node devops/tools/scripts/migrate-custom-piece-to-turbo.ts
```

This scans `backend/packages/pieces/custom/` and applies all necessary changes.
...
```bash
npx ts-node devops/tools/scripts/migrate-custom-piece-to-turbo.ts backend/packages/pieces/custom/my-piece
```
...
The build output should appear in `backend/packages/pieces/custom/your-piece/dist/`.
