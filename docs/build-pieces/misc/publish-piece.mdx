...
The CLI scans the `backend/packages/pieces/` directory for the specified piece. It checks the **name** and **version** in the `package.json` file. If the piece is not already published, it builds, packages, and uploads it to the platform using the API.
...
In case you choose `Env Variable (AP_API_KEY)`, the CLI will use the API key from the `.env` file in the `backend/packages/server/api` directory.
