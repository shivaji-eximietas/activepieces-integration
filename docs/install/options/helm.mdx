...
Create a `my-values.yaml` file with your configuration. You can use the [example values file](https://github.com/activepieces/activepieces/blob/main/devops/deploy/activepieces-helm/values.yaml) as a reference.
...
```bash
helm install activepieces devops/deploy/activepieces-helm -f my-values.yaml
```
...
# Upgrade release
helm upgrade activepieces devops/deploy/activepieces-helm -f my-values.yaml
