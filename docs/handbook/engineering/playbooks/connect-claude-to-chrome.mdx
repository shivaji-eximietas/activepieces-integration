...
The devcontainer is already configured to set up the Chrome DevTools MCP automatically. The `postCreateCommand` in `devops/.devcontainer/devcontainer.json` installs dependencies and registers the MCP server, resolving the host IP via `host.docker.internal`:
